...
const static int ERROR_LEVEL = 3;
static int __error_temp = 0;
...
input double start_lots_val = 0.01e2;     // Starting Lot Size
...
input string GID = "6D99AF0B-8EA5-4E8A-82B4-1C0303F594C5";
string make_symbol_gid(string symbol = "")
{
    return GID + (symbol == "" ? Symbol() : symbol);
}
...
string PanelTitle = "Upcoming News Events"; // Panel titleclrBlack
...
int h_RSI, h_ADX, h_atrH1, h_atrH4, h_atrD1, h_atrW1, h_ma1, h_ma2, h_ma3;
...
{
    static double res[1];
    return CopyBuffer(h, 0, shift, 1, res) != 1 ? 0 : res[0];
}
...
bool ObjectSetText(string name, string text, int font_size, string font = "", color text_color = CLR_NONE)
{
    ...
    bool condition = ObjectSetString(0, name, OBJPROP_TEXT, text) && ObjectSetInteger(0, name, OBJPROP_FONTSIZE, font_size);
    if (StringLen(text) <= 0 || font_size <= 0 || condition && StringLen(font) > 0 && !ObjectSetString(0, name, OBJPROP_FONT, font))
    {
        return false;
    }
    return condition;
...
int OnInit(void)
{
    ...
    h_RSI = iRSI(Symbol(), rsi_tf, rsi_period, PRICE_CLOSE);
    h_ADX = iADX(Symbol(), ADX_Timeframe, ADX_Period);

    h_atrH1 = iATR(Symbol(), PERIOD_H1, 3);
    h_atrH4 = iATR(Symbol(), PERIOD_H4, 3);
    h_atrD1 = iATR(Symbol(), PERIOD_D1, 3);
    h_atrW1 = iATR(Symbol(), PERIOD_W1, 3);

    h_ma1 = iMA(Symbol(), EMA_Timeframe_Filter, ma1_period, 0, ma1_method, PRICE_CLOSE);
    h_ma2 = iMA(Symbol(), EMA_Timeframe_Filter, ma2_period, 0, ma2_method, PRICE_CLOSE);
    h_ma3 = iMA(Symbol(), EMA_Timeframe_Filter, ma3_period, 0, ma3_method, PRICE_CLOSE);
...
void OnDeinit(const int reason)
{
    ...
    if (IsTesting())
    {
        GlobalVariableDel(make_symbol_gid() + "-Phantom Buy");
        GlobalVariableDel(make_symbol_gid() + "-Phantom Sell");
        GlobalVariableDel(make_symbol_gid() + "-First Buy");
        GlobalVariableDel(make_symbol_gid() + "-First Sell");
        GlobalVariableDel(make_symbol_gid() + "-Trail Active Buy");
        GlobalVariableDel(make_symbol_gid() + "-Trail Active Sell");
        GlobalVariableDel(make_symbol_gid() + "-EP Hit");
        GlobalVariableDel(make_symbol_gid() + "-FB Hit");
        GlobalVariableDel("GEN" + GID + "-FB Hit");
        GlobalVariableDel(GID + "-Slice ON");
        GlobalVariableDel(make_symbol_gid() + "-Chop BUY reset");
        GlobalVariableDel(make_symbol_gid() + "-Chop BUY");
        GlobalVariableDel(make_symbol_gid() + "-Chop SELL reset");
        GlobalVariableDel(make_symbol_gid() + "-Chop SELL");
        GlobalVariableDel(make_symbol_gid() + "-LMM-BUY");
        GlobalVariableDel(make_symbol_gid() + "-LMM-SELL");
        GlobalVariableDel(make_symbol_gid() + "-PSM-BUY");
        GlobalVariableDel(make_symbol_gid() + "-PSM-SELL");
        GlobalVariableDel(make_symbol_gid() + "-TPM-BUY");
        GlobalVariableDel(make_symbol_gid() + "-TPM-SELL");
        GlobalVariableDel(make_symbol_gid() + "-FBM");
        GlobalVariableDel(make_symbol_gid() + "-FBM-Goal");
        GlobalVariableDel(make_symbol_gid() + "MR-Trail-HWM");
        GlobalVariableDel(make_symbol_gid() + "-CFM-BUY");
        GlobalVariableDel(make_symbol_gid() + "-CFM-SELL");
        GlobalVariableDel("GEN" + GID + "-CloseForWeek");
        GlobalVariableDel(make_symbol_gid() + "-PAUSE-BUY");
        GlobalVariableDel(make_symbol_gid() + "-PAUSE-SELL");
        GlobalVariableDel(make_symbol_gid() + "-PAUSE-ACTIVE");
        GlobalVariableDel("GEN" + GID + "-ACC_SL_HIT");
        GlobalVariableDel("DailyCheck" + GID);
        GlobalVariableDel("DailyStartBalance" + GID);
        GlobalVariableDel("DailyStartEquity" + GID);
        GlobalVariableDel(make_symbol_gid() + "PB_Goal_Reached");
        GlobalVariableDel("GEN" + GID + "-ACC_TP_HIT");
    }
}
...
{
    return GetValue(h_ADX) < ADX_Max_Value;
}
...
bool TradingSession()
{
    ...
    return (trade_Asia_Session && current_time >= Asia_start_time && current_time <= Asia_end_time) || (trade_London_Session && current_time >= London_start_time && current_time <= London_end_time) || (trade_NY_Session && current_time >= NY_start_time && current_time <= NY_end_time);
}
...
void OnTick()
{
    ...
    if (GlobalVariableGet(make_symbol_gid() + "-PAUSE-ACTIVE") <= 0)
    {
        GlobalVariableDel(make_symbol_gid() + "-PAUSE-BUY");
        GlobalVariableDel(make_symbol_gid() + "-PAUSE-SELL");
    }
    ...
    if ((GlobalVariableGet(make_symbol_gid() + "-PAUSE-BUY") == 1 || GlobalVariableGet(make_symbol_gid() + "-PAUSE-SELL") == 1) && ((resume_after_daily_goal == ep_next_day && currentTime >= start_of_day) || (resume_after_daily_goal == ep_next_week && TimeDayOfWeek(currentTime) == 1 && currentTime >= start_of_week)))
    {
        GlobalVariableSet(make_symbol_gid() + "-PAUSE-BUY", 0);
        GlobalVariableSet(make_symbol_gid() + "-PAUSE-SELL", 0);
    }
    ...
    if (use_adx_filter && !IsADXBelowThreshold())
    {
        if (ADX_Max_Value_Action == PAUSE_START_TRADES)
        {
            ...
            GlobalVariableSet(make_symbol_gid() + "-PAUSE-ACTIVE", 1);
        }
    ...
    if (buys >= cfm_start && use_candle_filter_manager)
        GlobalVariableSet(make_symbol_gid() + "-CFM-BUY", 1);
    if (sells >= cfm_start && use_candle_filter_manager)
        GlobalVariableSet(make_symbol_gid() + "-CFM-SELL", 1);
    ...
    if (use_fb_manager)
    {
        ...
        {
            GlobalVariableSet(make_symbol_gid() + "-FBM", 1);
        }
    }

    // Enable Lot Manager
    if (buys >= lmm_start && lmm_type == 0 && use_lot_multiplier_manager)
        GlobalVariableSet(make_symbol_gid() + "-LMM-BUY", 1);
    if (sells >= lmm_start && lmm_type == 0 && use_lot_multiplier_manager)
        GlobalVariableSet(make_symbol_gid() + "-LMM-SELL", 1);
    ...
    if (buys >= psm_start && psm_type == 0 && use_pip_step_manager && GlobalVariableGet(make_symbol_gid() + "-PSM-BUY") <= 0)
        GlobalVariableSet(make_symbol_gid() + "-PSM-BUY", buys);
    if (sells >= psm_start && psm_type == 0 && use_pip_step_manager && GlobalVariableGet(make_symbol_gid() + "-PSM-SELL") <= 0)
        GlobalVariableSet(make_symbol_gid() + "-PSM-SELL", sells);
    ...
    if (use_tp_manager && buys >= tp_manager_start)
        GlobalVariableSet(make_symbol_gid() + "-TPM-BUY", 1);
    if (use_tp_manager && sells >= tp_manager_start)
        GlobalVariableSet(make_symbol_gid() + "-TPM-SELL", 1);
    if (use_tp_manager && buys < tp_manager_start)
        GlobalVariableDel(make_symbol_gid() + "-TPM-BUY");
    if (use_tp_manager && sells < tp_manager_start)
        GlobalVariableDel(make_symbol_gid() + "-TPM-SELL");

    phantom_buy = GlobalVariableGet(make_symbol_gid() + "-Phantom Buy");
    phantom_sell = GlobalVariableGet(make_symbol_gid() + "-Phantom Sell");
    pb_goal_reached = GlobalVariableGet(make_symbol_gid() + "PB_Goal_Reached");
    bool fb_hit = (GlobalVariableGet("GEN" + GID + "-FB Hit") >= 1 || GlobalVariableGet(make_symbol_gid() + "-FB Hit") >= 1);
    bool weekly_goal_hit = WeekProfitPerc() >= weekly_goal_perc;
    bool equity_protector_hit = GlobalVariableGet(make_symbol_gid() + "-EP Hit") > 0;
    bool slice_on = GlobalVariableGet(GID + "-Slice ON") > 0;
    bool chop_buy_active = GlobalVariableGet(make_symbol_gid() + "-Chop BUY reset") > 0;
    bool chop_sell_active = GlobalVariableGet(make_symbol_gid() + "-Chop SELL reset") > 0;
    bool close_for_week = GlobalVariableGet("GEN" + GID + "-CloseForWeek") > 0;
    bool pause_buy = GlobalVariableGet(make_symbol_gid() + "-PAUSE-BUY") > 0;
    bool pause_sell = GlobalVariableGet(make_symbol_gid() + "-PAUSE-SELL") > 0;
    bool acc_sl_hit = GlobalVariableGet("GEN" + GID + "-ACC_SL_HIT") > 0;
    bool acc_tp_hit = GlobalVariableGet("GEN" + GID + "-ACC_TP_HIT") > 0;
        ...
        ((!slice_on && use_slice_mode) || !use_slice_mode))
    {
        ...
        if (isTradingEnabled &&
            (!GlobalVariableGet(make_symbol_gid() + "-PAUSE-BUY")) && // Replace pause_buy
            buys <= 0 &&
            ...
            ((MA_Entry_Direction == 1 && IsUptrend()) || (MA_Entry_Direction == -1 && !IsUptrend()))) // Add MA_Entry_Direction check
        {
            ...
            {
                GlobalVariableSet(make_symbol_gid() + "-First Buy", buy);
                GlobalVariableDel(make_symbol_gid() + "-Trail Active Buy");
            }
        }
        if (isTradingEnabled &&
            (!GlobalVariableGet(make_symbol_gid() + "-PAUSE-SELL")) && // Replace pause_sell
            sells <= 0 &&
...

        {
            ...
            {
                GlobalVariableSet(make_symbol_gid() + "-First Sell", sell);
                GlobalVariableDel(make_symbol_gid() + "-Trail Active Sell");
            }
        }
    }
...
bool CANDLE_BUY()
{
    ...
    if (GlobalVariableGet(make_symbol_gid() + "-CFM-BUY") > 0)
...
bool CANDLE_SELL()
{
    ...
    ENUM_TIMEFRAMES new_tf = candle_filter_tf;
    if (GlobalVariableGet(make_symbol_gid() + "-CFM-SELL") > 0)
        new_tf = cfm_timeframe;
...
void CheckNewLevels()
{
    double Buys[];
    double Sells[];
    bool buy_cut = GlobalVariableGet(make_symbol_gid() + "-Buy CUT") > 0;
    bool sell_cut = GlobalVariableGet(make_symbol_gid() + "-Sell CUT") > 0;
    ...
    if (pip_step != 2)
    {
        if (lastbuyticket > 0 && ArraySize(Buys) < max_levels && CANDLE_BUY_ACTIVE())
        {
            ...
            if (ArraySize(Buys) > 1 && pip_step_multiplier != 1 && GlobalVariableGet(make_symbol_gid() + "-PSM-BUY") <= 0)
                next_pip_step *= ((ArraySize(Buys) - 1) * pip_step_multiplier);
            if (GlobalVariableGet(make_symbol_gid() + "-PSM-BUY") > 0)
            {
                int str_lvls = psm_interval;
                if (str_lvls <= 0)
                    str_lvls = 1;
                int lvls = (int)((ArraySize(Buys) - GlobalVariableGet(make_symbol_gid() + "-PSM-BUY")) / str_lvls);
            ...
            if (Ask <= o_position.PriceOpen() - next_pip_step && (pip_step == 0 || pip_step == 1))
            {
                ...
                if (GlobalVariableGet(make_symbol_gid() + "-Chop BUY") > 0)
                {
                    ChopBuys();
                }
                else
        ...
        {
            double next_pip_step = GetPipStep();
            if (ArraySize(Sells) > 1 && pip_step_multiplier != 1 && GlobalVariableGet(make_symbol_gid() + "-PSM-BUY") <= 0)
                next_pip_step *= ((ArraySize(Sells) - 1) * pip_step_multiplier);

            if (GlobalVariableGet(make_symbol_gid() + "-PSM-SELL") > 0)
            {
                int str_lvls = psm_interval;
                if (str_lvls <= 0)
                    str_lvls = 1;
                int lvls = (int)((ArraySize(Sells) - GlobalVariableGet(make_symbol_gid() + "-PSM-SELL")) / str_lvls);
                for (int v = 1; v <= lvls; v++)
                    next_pip_step *= psm_multiplier;
            }
            ...
            if (Bid >= o_position.PriceOpen() + next_pip_step && (pip_step == 0 || pip_step == 1))
            {
                ...
                if (GlobalVariableGet(make_symbol_gid() + "-Chop SELL") > 0)
                {
                    ChopSells();
                }
                else
    ...
    if (use_cut_mode)
    {
        if (GlobalVariableGet(make_symbol_gid() + "-Buy CUT") > 0 && cut_the_buy)
        {
        ...
        if (GlobalVariableGet(make_symbol_gid() + "-Sell CUT") > 0 && cut_the_sell)
        {
...
void CheckNewPhantomLevels()
{
    ...
    if ((Ask <= lowestbuy - phantom_pip_step * Point() || Buys <= 0) && Buys < phantom_levels && GlobalVariableGet(make_symbol_gid() + "-Phantom Buy") <= 0)
    {
        // Print("BUY");
    ...
    if ((Bid >= highestsell + phantom_pip_step * Point() || Sells <= 0) && Sells < phantom_levels && GlobalVariableGet(make_symbol_gid() + "-Phantom Sell") <= 0)
    {
        // Print("SELL");
    ...
    if ((Ask <= lowestbuy - phantom_pip_step * Point()) && Buys >= phantom_levels)
    {
        ...
        if (phantom_direction == 0)
            GlobalVariableSet(make_symbol_gid() + "-Phantom Buy", 1);
        if (phantom_direction == 1)
            GlobalVariableSet(make_symbol_gid() + "-Phantom Sell", 1);
    }
...
double GetNewLevelLots(string dir, int level, double s_lots)
{
    ...
    if (dir == "BUY")
    {
        if (GlobalVariableGet(make_symbol_gid() + "-LMM-BUY") > 0)
        {
            double last_lot = 0;
            datetime lastopen = 0;
            for (int i = PositionsTotal() - 1; i >= 0; i--)
            ...
            if (dir == "BUY" && GlobalVariableGet(make_symbol_gid() + "-LMM-BUY") > 0)
            {
                last_lot *= lmm_multipler;
            }
            if (dir == "SELL" && GlobalVariableGet(make_symbol_gid() + "-LMM-SELL") > 0)
            {
                last_lot *= lmm_multipler;
            }
            if (dir == "BUY" && GlobalVariableGet(make_symbol_gid() + "-LMM-BUY") <= 0)
    ...
    if (dir == "SELL")
    {
        if (GlobalVariableGet(make_symbol_gid() + "-LMM-SELL") > 0)
        {
            double last_lot = 0;
            datetime lastopen = 0;
...
double GetPipStep()
{
    ...
    if (pip_step == 1)
    {
        double atrH1 = GetValue(h_atrH1); // iATR(Symbol(), PERIOD_H1, 3, 0);
        double atrH4 = GetValue(h_atrH4); // iATR(Symbol(), PERIOD_H4, 3, 0);
        double atrD1 = GetValue(h_atrD1); // iATR(Symbol(), PERIOD_D1, 3, 0);
        double atrW1 = GetValue(h_atrW1); // iATR(Symbol(), PERIOD_W1, 3, 0);
        double average = (atrH1 + atrH4 + atrD1 + atrW1) / 4;
        return average / pip_step_amount;
    }
    return pip_step_amount * Point();
}
// +------------------------------------------------------------------+
...
bool IsUptrend()
{
    ...
    double ma1 = GetValue(h_ma1, 1); // iMA(Symbol(), EMA_Timeframe_Filter, ma1_period, 0, ma1_method, PRICE_CLOSE, 1);
    double ma2 = GetValue(h_ma2, 1); // iMA(Symbol(), EMA_Timeframe_Filter, ma2_period, 0, ma2_method, PRICE_CLOSE, 1);
    double ma3 = GetValue(h_ma3, 1); // iMA(Symbol(), EMA_Timeframe_Filter, ma3_period, 0, ma3_method, PRICE_CLOSE, 1);
...
bool IsPriceCrossingMA()
{
    ...
    double Close = iClose(Symbol(), 0, 0);
    double MA = GetValue(h_ma1, 0); // iMA(Symbol(), EMA_Timeframe_Filter, ma1_period, 0, ma1_method, PRICE_CLOSE, 0);
    double PrevClose = iClose(Symbol(), 0, 1);
    double PrevMA = GetValue(h_ma1, 1); // iMA(Symbol(), EMA_Timeframe_Filter, ma1_period, 0, ma1_method, PRICE_CLOSE, 1);
...
bool MA_BUY_ENTRY()
{
    ...
    {
        double Close = iClose(Symbol(), 0, 0);
        double FastMA = GetValue(h_ma1, 0);
        double MediumMA = GetValue(h_ma2, 0);
        double SlowMA = GetValue(h_ma3, 0);
...

{
    ...
    if (MA_Entry_Direction == -1 || MA_Entry_Direction == 1) // Trading against the trend
    {
        ...
        if (use_ma1 && use_ma2 && use_ma3)
        {
            if ((GetValue(h_ma2, 0) < GetValue(h_ma3, 0)))
            {
                last_trade_time = TimeCurrent(); // Update last_trade_time
                opened_positions++;              // Increment the number of opened positions
                return true;
...
bool ADX_BUY_ENTRY()
{
    // If the ADX filter is not activated
    if (!use_adx_filter || IsADXBelowThreshold())
    {
        return true; // Allow trading
    }
    // Handle the max value action
    if (ADX_Max_Value_Action == PAUSE_START_TRADES || ADX_Max_Value_Action != CLOSE_ALL_TRADES)
    {
        return false; // Pause opening new trades
    }
    // Close all trades
    for (int i = PositionsTotal() - 1; i >= 0; i--)
    {
...
bool SPREAD_GO()
{
    return SymbolSpread() <= max_spread;
}
...
void ManageTP_BUYS()
{
    ...
    if (opentrades >= 1)
    {
        ...
        if (GlobalVariableGet(make_symbol_gid() + "-TPM-BUY") > 0)
        {
        ...
        int first_buy = (int)GlobalVariableGet(make_symbol_gid() + "-First Buy");
        bool buy_trail_active = GlobalVariableGet(make_symbol_gid() + "-Trail Active Buy") >= 1;
...
void ManageTP_SELLS()
{
    ...
    if (opentrades >= 1)
    {
        ...
        if (GlobalVariableGet(make_symbol_gid() + "-TPM-SELL") > 0)
        {
            int tpm_levels = (int)(opentrades - tp_manager_start);
            NewTP = (numerator / denominator) - (tp_manager_new_tp * Point());
        ...
        int first_sell = (int)GlobalVariableGet(make_symbol_gid() + "-First Sell");
        bool sell_trail_active = GlobalVariableGet(make_symbol_gid() + "-Trail Active Sell") >= 1;
        for (int u = PositionsTotal() - 1; u >= 0; u--)
        {
            if (!o_position.SelectByIndex(u))
            {
...
void CheckAccountTP()
{
    ...
    {
        GlobalVariableSet(make_symbol_gid() + "-PAUSE-BUY", 1);
        GlobalVariableSet(make_symbol_gid() + "-PAUSE-SELL", 1);
        for (int i = PositionsTotal() - 1; i >= 0; i--)
        {
            if (!o_position.SelectByIndex(i))
            {
        ...
        if (GlobalVariableGet("GEN" + GID + "-ACC_TP_HIT") <= 0)
            Alert("Account TP Hit!");
        GlobalVariableSet("GEN" + GID + "-ACC_TP_HIT", 1);
...
void CheckFloatingBucket()
{
    ...
    if (GlobalVariableGet(make_symbol_gid() + "-FBM") > 0)
    {
        ...
        if (new_fb_goal > GlobalVariableGet(make_symbol_gid() + "-FBM-Goal"))
        {
            new_fb_goal = GlobalVariableGet(make_symbol_gid() + "-FBM-Goal");
        }
        else
        {
            GlobalVariableSet(make_symbol_gid() + "-FBM-Goal", new_fb_goal);
        }
    }
    ...
    if (fb_start > 0)
    {
        if (profit >= fb_start)
        {
            if (profit > GlobalVariableGet(make_symbol_gid() + "MR-Trail-HWM"))
            {
                GlobalVariableSet(make_symbol_gid() + "MR-Trail-HWM", profit);
            }
        }
        else
        {
            GlobalVariableSet(make_symbol_gid() + "MR-Trail-HWM", 0);
        }
    }
    ...
    if (fb_type == pair)
    {
        ...
        else if (fb_start > 0 && GlobalVariableGet(make_symbol_gid() + "MR-Trail-HWM") > 0 &&
                 profit <= (GlobalVariableGet(make_symbol_gid() + "MR-Trail-HWM") - fb_stop))
        {
            isGoalReached = true;
        }
    }
    ...
    {
        if (profit >= new_fb_goal || (fb_start > 0 && GlobalVariableGet(make_symbol_gid() + "MR-Trail-HWM") > 0 &&
                                      profit <= (GlobalVariableGet(make_symbol_gid() + "MR-Trail-HWM") - fb_stop)))
        {
            isGoalReached = true;
        }
    ...
    else if (fb_type == account_perc)
    {
        if (floatingProfitPercent >= new_fb_goal || (fb_start > 0 &&
                                                     GlobalVariableGet(make_symbol_gid() + "MR-Trail-HWM") > 0 &&
                                                     profit <= (GlobalVariableGet(make_symbol_gid() + "MR-Trail-HWM") - fb_stop)))
        {
            isGoalReached = true;
        }
    ...
    if (isGoalReached)
    {
        ...
        if (!fb_auto_reset)
        {
            if (GlobalVariableGet(make_symbol_gid() + "-FB Hit") <= 0)
            {
                Alert("Floating Bucket Hit!");
            }
            GlobalVariableSet(make_symbol_gid() + "-FB Hit", 1);
        }
    }
...
void CheckDailyGoal()
{
...

    {
        ...
        if (GlobalVariableGet(make_symbol_gid() + "-PAUSE-BUY") <= 0)
            Alert("Daily Goal Hit!");

        GlobalVariableSet(make_symbol_gid() + "-PAUSE-BUY", 1);
        GlobalVariableSet(make_symbol_gid() + "-PAUSE-SELL", 1);
        GlobalVariableSet(make_symbol_gid() + "-PAUSE-ACTIVE", 1); // Include this line to also set the -PAUSE-ACTIVE variable
    }
}
...
void CheckAccountSL()
{
    ...
    if (AccountEquity() <= acc_sl)
    {
        ...
        if (GlobalVariableGet("GEN" + GID + "-ACC_SL_HIT") <= 0)
            Alert("Account SL Hit!");
        GlobalVariableSet("GEN" + GID + "-ACC_SL_HIT", 1);
                  ...
                  const string &sparam) // Parameter of type string events)
{
    ...
    if (id == CHARTEVENT_OBJECT_CLICK)
    {
        ...
        {
            GlobalVariableDel(make_symbol_gid() + "MR_PB_Start_Time");
            GlobalVariableDel(make_symbol_gid() + "PB_Goal_Reached");
            m_pb_reset_button.Color(clrGray);
        }
...
{
    if (!GlobalVariableCheck(make_symbol_gid() + "MR_PB_Start_Time"))
    {
        GlobalVariableSet(make_symbol_gid() + "MR_PB_Start_Time", TimeCurrent());
    }
}
// +------------------------------------------------------------------+
void CheckProfitBucket()
{
    ...
    datetime t = (datetime)GlobalVariableGet(make_symbol_gid() + "MR_PB_Start_Time");
    ...
    for (int i = PositionsTotal() - 1; i >= 0; i--)
    {
        ...
        if (
            o_position.Time() >= GlobalVariableGet(make_symbol_gid() + "MR_PB_Start_Time") &&
    ...
    if (profit >= profit_bucket_goal)
    {
        if (profit_bucket_auto_reset)
        {
            ...
            GlobalVariableSet(make_symbol_gid() + "PB_Goal_Reached", 1);
        for (int i = PositionsTotal() - 1; i >= 0; i--)
        {
            if (!o_position.SelectByIndex(i))
            {
...
double WeekProfitPerc()
{
    ...
    for (int i = HistoryOrdersTotal() - 1; i >= 0; i--)
    {
        ...
        if (o_history.Magic() == mn && o_history.TimeDone() >= startofweek)
        {
            profit += o_deal.Profit() + o_deal.Commission() + o_deal.Swap();
        }
    }
        ...
        if (o_position.Magic() == mn)
        {
                ...
                o_position.PositionType() == POSITION_TYPE_BUY)
            {
                ...
                if (NewStop != 0)
                {
                    ...
                    GlobalVariableSet(make_symbol_gid() + "-Trail Active Buy", 1);
                }
            }
                ...
                o_position.PositionType() == POSITION_TYPE_SELL)
            {
                ...
                {
                    ModifyStopLoss(NewStop);
                    GlobalVariableSet(make_symbol_gid() + "-Trail Active Sell", 1);
                }
            }
        }
    }
                         ...
                         color arrow_color = CLR_NONE)
{
    ...
    if (!IsConnected())
    {
        ...
        __error_temp = ERR_NO_CONNECTION;
        return (false);
    }
    ...
    if (!IsTradeAllowed())
    {
        ...
        __error_temp = ERR_TRADE_CONTEXT_BUSY;
        return (false);
    }
    ...
    if (false)
    {
        ...
        if (stoploss != 0)
            OrderReliable_EnsureValidSL(Symbol(), price, stoploss);
        if (takeprofit != 0)
            OrderReliable_EnsureValidTP(Symbol(), price, takeprofit);
    }
...
void CheckEquityProtector()
{
    ...
    if (GlobalVariableGet(make_symbol_gid() + "-EP Hit") > 0)
    {
    }
    else
    {
        ...
        (equity_protector_type == ep_MDL_Highest_Value && MDL_floating_loss <= (equity_protector_value * -1)))
    {
        ...
        if (equity_protector_auto_resume_type == ep_next_day)
        {
            ...
            {
                GlobalVariableSet(make_symbol_gid() + "-EP Hit", currentTime);
        ...
        if (equity_protector_auto_resume_type == ep_next_week)
        {
            ...
            {
                GlobalVariableSet(make_symbol_gid() + "-EP Hit", 0);
...
void ResetEquityProtector()
{
    ...
    if (
        (
            (equity_protector_type == ep_MDL_Balance || equity_protector_type == ep_MDL_Equity || equity_protector_type == ep_MDL_Highest_Value) && equity_protector_auto_resume_type == ep_next_day && (TimeHour(currentTime) >= 0 && TimeHour(currentTime) <= 1) && TimeMinute(currentTime) < 5 && currentTime >= (GlobalVariableGet(make_symbol_gid() + "-EP Hit") + TimeDiff) + PERIOD_D1) ||
        ((equity_protector_type == ep_account_percent || equity_protector_type == ep_floating_loss) && equity_protector_auto_resume_type == ep_next_day && TimeHour(iTime(Symbol(), PERIOD_D1, 0)) == 0 && TimeMinute(iTime(Symbol(), PERIOD_D1, 0)) < 5 && iTime(Symbol(), PERIOD_D1, 0) >= GlobalVariableGet(make_symbol_gid() + "-EP Hit") + PERIOD_D1))
    {
        GlobalVariableDel(make_symbol_gid() + "-EP Hit");
...
void CheckSliceON()
{
    ...
    {
        GlobalVariableSet(GID + "-Slice ON", 1);
...
void UpdateCut(int buys, int sells)
{
    if (buys >= begin_cut_level)
        GlobalVariableSet(make_symbol_gid() + "-Buy CUT", 1);
    if (sells >= begin_cut_level)
        GlobalVariableSet(make_symbol_gid() + "-Sell CUT", 1);
    if (buys <= 0)
        GlobalVariableDel(make_symbol_gid() + "-Buy CUT");
    if (sells <= 0)
        GlobalVariableDel(make_symbol_gid() + "-Sell CUT");
}
// +------------------------------------------------------------------+
void Chop()
{
    ...
    if (buys >= chop_level)
    {
        GlobalVariableSet(make_symbol_gid() + "-Chop BUY", 1);
    }
    else
        GlobalVariableDel(make_symbol_gid() + "-Chop BUY");
    if (sells >= chop_level)
    {
        GlobalVariableSet(make_symbol_gid() + "-Chop SELL", 1);
    }
    else
        GlobalVariableDel(make_symbol_gid() + "-Chop SELL");
}
...
void ChopBuys()
{
    GlobalVariableSet(make_symbol_gid() + "-Chop BUY reset", TimeCurrent());
    for (int i = PositionsTotal() - 1; i >= 0; i--)
    {
        if (!o_position.SelectByIndex(i))
        {
...
void ChopSells()
{
    GlobalVariableSet(make_symbol_gid() + "-Chop SELL reset", TimeCurrent());
    for (int i = PositionsTotal() - 1; i >= 0; i--)
    {
        if (!o_position.SelectByIndex(i))
        {
...
void ResetChop()
{
    if (GlobalVariableGet(make_symbol_gid() + "-Chop BUY reset") > 0 && TimeCurrent() >= GlobalVariableGet(make_symbol_gid() + "-Chop BUY reset") + resume_chop * 60)
    {
        GlobalVariableDel(make_symbol_gid() + "-Chop BUY reset");
        GlobalVariableDel(make_symbol_gid() + "-Chop BUY");
    }
    if (GlobalVariableGet(make_symbol_gid() + "-Chop SELL reset") > 0 && TimeCurrent() >= GlobalVariableGet(make_symbol_gid() + "-Chop SELL reset") + resume_chop * 60)
    {
        GlobalVariableDel(make_symbol_gid() + "-Chop SELL reset");
        GlobalVariableDel(make_symbol_gid() + "-Chop SELL");
    }
...
void CloseAllTradesOnTime(int dayOfWeek, string time)
{
    ...
    {
        GlobalVariableSet("GEN" + GID + "-CloseForWeek", 1);
...
void UpdatePanelLabels()
{
    ...
    if (ArraySize(Sells) > 1 && pip_step_multiplier != 1 && GlobalVariableGet(make_symbol_gid() + "-PSM-SELL") <= 0)
    ...
    if (GlobalVariableGet(make_symbol_gid() + "PB_Goal_Reached") > 0)
    {
    }
}
...
void CheckDailyValues()
{
    ...
    if (today > GlobalVariableGet("DailyCheck" + GID))
    {
        GlobalVariableSet("DailyCheck" + GID, today);
        GlobalVariableSet("DailyStartBalance" + GID, AccountBalance());
        GlobalVariableSet("DailyStartEquity" + GID, AccountEquity());
    }
    m_daily_start_label.Text("Daily Start Balance: $" + DoubleToString(GlobalVariableGet("DailyStartBalance" + GID), 2));
}
...
void UpdateNews()
{
    ...
    if (!news_impact)
        GlobalVariableDel(make_symbol_gid() + "-News_Impact");
}
...
void DrawEvents()
{
    ...
    for (int i = 0; i < 5; i++)
    {
        ...
        else if (ePrevious[i] < eForecast[i])
        ...
        if (ShowPanel)
        {
            ...
            Draw("Event " + (string)i,
                 DayToStr(eTime[i]) + "  |  " +
                     ...
                     eCountry[i] + "  |  " +
                     eTitle[i],
                 FontSize, FontName, EventColor, Corner, x2_, 70 - i * 15, eToolTip);
            // --- forecast
...
{
    return ReportForUSD && symbol == "USD" || ReportForGBP && symbol == "GBP" || ReportForEUR && symbol == "EUR" || ReportForCAD && symbol == "CAD" || ReportForAUD && symbol == "AUD" || ReportForCHF && symbol == "CHF" || ReportForJPY && symbol == "JPY" || ReportForNZD && symbol == "NZD" || ReportForCNY && symbol == "CNY";
}
// +------------------------------------------------------------------+
